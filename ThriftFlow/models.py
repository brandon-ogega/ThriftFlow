...
class IncomeAllocation(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    income_source = models.ForeignKey(IncomeSource, on_delete=models.CASCADE, related_name='allocations')
...
class Bill(models.Model):
    ...
    def __str__(self):
        recurring_str = "Recurring" if self.recurring else "One-time"
        return f"{self.title} - {self.amount} ({recurring_str})"
